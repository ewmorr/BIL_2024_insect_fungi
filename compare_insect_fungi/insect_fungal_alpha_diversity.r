##############################################################################
# Insect vs. Fungal Alpha Diversity
#
...
# (insect_exploratory/insect_ords.R, compare_insect_fungi/
# fungal_community_seasonality.r)?
#
# Community tables match the ordinations exactly, same as every other script
# in compare_insect_fungi/:
# - Insect: the raw Curculionidae+Latridiidae trap-catch table used directly
#   in insect_ords.R / fig2's insect NMDS (no rarefaction -- trap catch counts
#   aren't subject to the same sequencing-depth artifact as amplicon reads).
# - Fungal: the same multiple-subsampling (rarefy to depth 5000 x 100
#   iterations) approach used for the fungal NMDS/PERMANOVA
#   (fungal_community_seasonality.r, fig2_nmds_procrustes.R), but averaging
#   per-sample ALPHA diversity across iterations instead of averaging
#   Bray-Curtis distance matrices -- the natural alpha-diversity analog of
#   the same rarefy-and-average strategy.
#
...
out_data_dir <- "data/compare_insects_fungi_alpha_diversity"
out_fig_dir <- "figures/compare_insects_fungi_alpha_diversity"
dir.create(out_data_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(out_fig_dir, showWarnings = FALSE, recursive = TRUE)

## ---- 1. Load and match insect + fungal data (same as fig2/compare_insect_fungi/*) ----
...
## ---- 3. Insect alpha diversity (raw trap-catch table, same as the ordination) ----
...
dist_plot <- ggplot(long_alpha, aes(x = community, y = value, fill = community)) +
  ...
  labs(x = NULL, y = NULL, title = "Alpha diversity by community") +
  alpha_theme
...
# y-scale -- insect richness (0-20) and fungal richness (0-900) would
# otherwise share one axis per row under facet_grid(scales="free_y"), which
# only frees scales across rows, not across columns within a row.
site_lure_plot <- ggplot(site_lure_df, aes(x = site, y = value, fill = lure)) +
  ...
  labs(x = "Site", y = NULL, title = "Alpha diversity by site and lure") +
  alpha_theme +
...
date_plot <- ggplot(site_lure_df, aes(x = date, y = value, fill = site)) +
  ...
  labs(x = "Collection date", y = NULL, title = "Alpha diversity vs. collection date") +
  alpha_theme
...
corr_grid <- gridExtra::grid.arrange(grobs = corr_panels, ncol = 3,
                                      top = "Insect vs. fungal alpha diversity (per-sample)")
