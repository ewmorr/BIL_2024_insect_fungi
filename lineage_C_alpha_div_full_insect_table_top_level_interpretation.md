# Interpretation update (2026-09-01, extended 2026-09-08, §7-12 corrected 2026-09-09, §13 added 2026-09-10, §14 added 2026-09-15): top-line numbers for the full-insect-table alpha-diversity lineage (Lineage C)
...
## 13. Total insect abundance vs. fungal alpha diversity -- the non-targeted-dispersal-vector hypothesis (added 2026-09-10)

§2 tests whether diverse insect catches carry diverse fungal communities (a partner-specificity idea) and finds them nearly decoupled. A different mechanism -- insects as **non-targeted dispersal vectors** that pick up whatever fungal propagules they contact as they move through the environment -- predicts that the relevant insect quantity is total *individuals* caught, not total *taxa*: more insect traffic = more independent draws from the environmental fungal pool = more fungal taxa detected. Predicted sign **positive**, strongest for fungal richness.

Test: `compare_insect_fungi/insect_abundance_fungal_alpha_diversity.full_insect_table.r`. Insect abundance = `rowSums()` of the same raw all-families 426-taxon Lineage C table; fungal alpha diversity read from `fungal_alpha_diversity.csv` / `fungal_asymptotic_diversity.csv` (not recomputed). Two analyses per metric: `raw_pooled` (plain Spearman, as §2/§5) and `season_residualized` (residualize `log1p(total_individuals)` and the fungal metric on `site + factor(date)` -- the `.residualized.r` convention -- then Spearman on residuals, p by 9999 free permutations). 3 metrics x 2 analyses x {observed, asymptotic}, uncorrected p.

| Fungal metric (observed, rarefied) | raw_pooled rho (p_perm) | season-resid rho (p_perm) |
|---|---|---|
| Shannon diversity | -0.02 (0.84) | +0.08 (0.50) |
| Simpson dominance | +0.02 (0.87) | -0.11 (0.38) |
| Richness | +0.08 (0.49) | +0.09 (0.48) |

Asymptotic (Chao) fungal diversity: same, every |rho| <= 0.08, every p_perm >= 0.52. **Flat null on every metric, both analyses, both value types.** The richness rows -- where the hypothesis predicts the clearest positive signal -- sit at rho ~ +0.08.

The null is informative rather than merely underpowered: total insect abundance has a wide spread (6-857 individuals/sample, median 101) and behaves sensibly against everything else -- rho=0.70 with insect richness (more individuals -> more insect taxa observed), and, unusually for this dataset, **essentially flat with collection date** (rho=0.01) where insect richness/composition and every fungal alpha metric are strongly seasonal (§4). So raw and season-residualized results agree, and neither shows anything. Fungal alpha diversity in these samples is set by site and season, not by how much insect biomass passed through the trap.

Files: `insect_abundance_fungal_alpha_correlation.csv`, `insect_abundance_by_sample.csv`, `insect_abundance_context_correlations.csv` in `data/compare_insects_fungi_alpha_diversity_full_insect_table/`; `figures/compare_insects_fungi_alpha_diversity_full_insect_table/insect_abundance_vs_fungal_alpha.{png,pdf}`.

## 14. Does lure affect insect trap sample COVERAGE (sampling efficiency)? Yes -- Ips lure specifically (added 2026-09-15)

§4/§10 already test lure against each *diversity* metric. This asks a different question: does lure affect how COMPLETE a sample a trap draws, independent of how diverse the catch turns out to be -- i.e. lure's effect on the Good-Turing `coverage` estimate itself (the same quantity §7-9's extrapolation is correcting for). One trap per site x lure (69 samples across 12 traps, 4 sites x 3 lures), so lure is constant within trap and can't be tested with a naive one-way ANOVA across all samples without pseudoreplicating and ignoring the site confound.

Test: `compare_insect_fungi/insect_coverage_lure_anova.full_insect_table.r`. Reads `coverage` straight from `insect_asymptotic_diversity.csv` (no recomputation). Omnibus permutation ANOVA (`coverage ~ site + lure + date` vs. `coverage ~ site + date`, lure permuted among traps within site, 999 perms) plus pairwise post-hoc lure contrasts extracted from that same full-model permutation null (site/date-adjusted lure-coefficient differences, BH-corrected across 3 pairs) -- a first attempt refitting the nested-model F-test separately on each 2-lure subset gave incoherent results (pairwise p's of 0.06-0.13 despite omnibus p=0.005) because restricting to 2 lures leaves only 2 traps per site to permute (2^4=16 configurations, too coarse); reusing the full 3-lure model's 6^4=1296-configuration null resolved this.

| Comparison | Result |
|---|---|
| Omnibus (lure term) | F=12.91, p_perm=0.005, r2=0.29 |
| Alpha-pinene_EtOH vs. Ethanol | adj. mean diff -0.028, p=0.760, q=0.760 |
| Alpha-pinene_EtOH vs. Ips | adj. mean diff +0.143, p=0.034, q=0.051 |
| Ethanol vs. Ips | adj. mean diff +0.171, p=0.002, q=0.006 |

Raw group means: Ethanol 0.834, Alpha-pinene_EtOH 0.806, Ips 0.658. **The omnibus lure effect is driven entirely by Ips catching a less complete sample than either of the other two lures**, which don't differ from each other. This is a sampling-efficiency finding about how thoroughly each lure's trap draws from the local community, distinct from (and not previously isolated from) whether Ips also yields lower estimated diversity outright (§4/§10) -- a lower-coverage sample would itself be expected to show lower *observed* diversity even before any true difference in the underlying community, which is part of why the asymptotic correction exists in the first place.

Files: `insect_coverage_lure_anova.csv`, `insect_coverage_lure_pairwise.csv` in `data/compare_insects_fungi_asymptotic_richness_iNEXT_full_insect_table/`; `figures/compare_insects_fungi_asymptotic_richness_iNEXT_full_insect_table/insect_coverage_by_lure.{png,pdf}`.
...
11. **Total insect abundance does not predict fungal alpha diversity either** (§13, added 2026-09-10) -- the "insects as non-targeted dispersal vectors" hypothesis predicts total individuals caught (not taxa) should correlate positively with fungal richness. It doesn't: every fungal metric, raw and season-residualized, observed and asymptotic, gives |rho| <= 0.11 and p_perm >= 0.38. Insect abundance is itself essentially flat with collection date (rho=0.01), so this isn't a seasonality artifact in either direction -- fungal alpha diversity here tracks site and season, not insect traffic through the trap.
12. **Lure affects insect trap sample COVERAGE, not just diversity** (§14, added 2026-09-15) -- a permutation ANOVA on the Good-Turing coverage estimate itself (`coverage ~ site + lure + date`) is significant (F=12.91, p_perm=0.005), driven entirely by the **Ips lure drawing a less complete sample** than Alpha-pinene_EtOH (q=0.051) or Ethanol (q=0.006), which don't differ from each other. This is a sampling-efficiency effect distinct from point 3's diversity effect, and plausibly contributes to it: a lower-coverage catch would show reduced *observed* diversity independent of any true community difference, which is exactly what the asymptotic correction in §7-11 exists to address.

Underlying files: §1-6 -- `data/compare_insects_fungi_alpha_diversity_full_insect_table/insect_alpha_diversity.csv`, `fungal_alpha_diversity.csv`, `alpha_diversity_cross_community_correlation.csv`, `alpha_diversity_insect_fungal_interaction_by_lure.csv`, `alpha_diversity_covariate_tests.csv`, `alpha_diversity_date_shape.csv`; figures in `figures/compare_insects_fungi_alpha_diversity_full_insect_table/` (descriptive) and `figures/presentation_items/fig9_alpha_diversity_by_lure.{png,pdf}`, `fig10_insect_fungal_alpha_diversity_correlation.{png,pdf}`. §7-12 -- `data/compare_insects_fungi_asymptotic_richness_iNEXT_full_insect_table/insect_asymptotic_diversity.csv`, `fungal_asymptotic_diversity.csv`, `asymptotic_diversity_cross_community_correlation.csv`, `asymptotic_diversity_insect_fungal_interaction_by_lure.csv`, `asymptotic_diversity_covariate_tests.csv`, `asymptotic_diversity_date_shape.csv`; figures in `figures/compare_insects_fungi_asymptotic_richness_iNEXT_full_insect_table/` (descriptive) and `figures/presentation_items/fig11_asymptotic_diversity_by_lure.{png,pdf}`, `fig12_insect_fungal_asymptotic_diversity_correlation.{png,pdf}`. §13 -- `data/compare_insects_fungi_alpha_diversity_full_insect_table/insect_abundance_{fungal_alpha_correlation,by_sample,context_correlations}.csv`; `figures/compare_insects_fungi_alpha_diversity_full_insect_table/insect_abundance_vs_fungal_alpha.{png,pdf}`. §14 -- `data/compare_insects_fungi_asymptotic_richness_iNEXT_full_insect_table/insect_coverage_lure_{anova,pairwise}.csv`; `figures/compare_insects_fungi_asymptotic_richness_iNEXT_full_insect_table/insect_coverage_by_lure.{png,pdf}`.
