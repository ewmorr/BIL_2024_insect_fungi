...
## 9. Diagnostic: does a taxon's collection window bias the date screen? (2026-09-15, cross-lineage -- covers §2's insect screen + the shared fungal screen)

Raised concern: the linear+quadratic date test (§2) fits a slope/curvature across the season, so a taxon confined to a narrow slice of it (e.g. caught on only the first 1-2 of the 6 collection dates) might carry an obvious early-/late-season signal the model has too little date-spread to resolve, and get misclassified "no significant date pattern" for a reason unrelated to whether it's actually seasonal.

New diagnostic script (`compare_insect_fungi/taxon_collection_window_vs_date_significance.r`) computed a seasonality-agnostic **collection window** (last collection date present − first date present, raw presence/absence) for every taxon in both this section's 153-taxon insect screen and the shared 6,342-taxon fungal screen, and tested it against the existing `date_significant` (q<0.10 linear or quadratic) call, adjusting for detection count (`n_samples_present`) since the two are naturally correlated (Spearman ~0.55-0.57).

**Result is the opposite of the concern**: at a fixed detection count, a *narrower* window makes a significant call MORE likely, not less (logistic model, insect b=-0.069 p=1.2e-6; fungal b=-0.035 p=2.3e-75) -- confirmed by crosstab: insect taxa present on only 1 of 6 dates are 5/5 (100%) significant vs. 15/31 (48%) for taxa spanning the full season. A narrow, concentrated run of detections is a stronger localized departure from baseline than the same few detections spread thinly across the whole season, which looks more like noise to the model. **The real limiting factor for a missed seasonal call is low overall detection count (rarity near the >=5-sample prevalence floor), not window narrowness** -- so §2's insect hit counts (and the shared fungal screen's) don't need revisiting on window-narrowness grounds. See the 2026-09-15 entry in `iterative_analysis_updates.md` for full numbers.
